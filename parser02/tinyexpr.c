...
enum {
    ...
    TOK_OPEN, TOK_CLOSE, TOK_NUMBER, TOK_VARIABLE, TOK_INFIX, TOK_FACT
};
...
te_expr *new_expr_value(double value) {
...
te_expr *te_compile(const char *expression, const te_variable *variables, int var_count, int *error) {
    ...
    if(s.type == TOK_FACT)
    {
        next_token(&s); // consume factorial operator
        te_expr *arg = list(&s); // compile factorial argument
        if (arg == NULL) {
            te_free(root);
            if (error) *error = -1;
            return NULL;
        }
        double value = te_eval(arg); // evaluate factorial argument
        te_free(arg);
        if (isnan(value)) {
            te_free(root);
            if (error) *error = -1;
        ...
        double fact = factorial(value); // calculate factorial value
        te_free(root);
        root = new_expr_value(fact); // create new expression with factorial value
    }
